...
//
// Read-only: no rows are created, updated or deleted. Asserts the page renders
// the LATEST analytics_insights row whose agent_type='policy_compliance_auditor'
// -- cases audited, total breaches, the three-check breakdown and the
// recommendation narrative -- and that #14's row still drives Reports, so the
// two agents' rows in the same table are not being confused.
//
//   node scripts/verify_policy_audit_view.cjs [baseUrl]
...
async function run() {
  ...
  console.log(`       latest audit ${audit.id.slice(0, 8)} @ ${audit.created_at} — ${stats.cases_audited} cases, ${stats.breach_count} breaches`)
  ...
  try {
    ...
    if (!cases?.includes(String(stats.cases_audited))) fail(`"Cases audited" shows ${cases}, DB says ${stats.cases_audited}`)
    else pass(`cases audited = ${stats.cases_audited}`)
    if (!breaches?.includes(String(stats.breach_count))) fail(`"Breaches found" shows ${breaches}, DB says ${stats.breach_count}`)
    ...
    for (const c of CHECKS) {
      ...
      if (shown !== String(byCheck[c] ?? 0)) fail(`${CHECK_LABEL[c]}: page shows ${shown}, DB says ${byCheck[c] ?? 0}`)
    }
    if (!failures.some((f) => f.includes('breakdown') || CHECKS.some((c) => f.startsWith(CHECK_LABEL[c])))) {
      pass(`breakdown matches the DB (${CHECKS.map((c) => `${c}=${byCheck[c] ?? 0}`).join(', ')})`)
    }

    if (view.strip !== audit.narrative) fail(`recommendation narrative does not match the DB row`)
    else pass(`recommendation narrative rendered (${audit.narrative.length} chars)`)

    const expectedDetail = Array.isArray(stats.breaches) ? stats.breaches.length : 0
    if (view.detailRows !== expectedDetail) fail(`breach detail shows ${view.detailRows} rows, DB row has ${expectedDetail}`)
