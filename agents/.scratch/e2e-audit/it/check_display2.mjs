...
for (const r of ['deprovisioning', 'audit-log', '']) {
  ...
  const text = await page.evaluate(() => {
    ...
    let el = header
    let out = []
    for (let i = 0; i < 10 && el.nextElementSibling; i++) {
...
await page.screenshot({ path: 'agents/.scratch/e2e-audit/screenshots/it/verification_failed_check.png', fullPage: true })
await browser.close()
