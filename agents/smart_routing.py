"""Agent #11 -- Smart Routing (agent_requirements.md #11 / blueprint1.md #11).
...
(Siva/Aravidhan/Aswin, scripts/seed.js) plus one REAL delegate account per
role (scripts/seed_delegates.js: hr.delegate@/manager.delegate@/
...
stage nodes (not a new top-level graph node) -- see agents/supervisor.py.

Run (from repo root, with agents/.venv active):
    python -m agents.smart_routing <case_id> <stage>   # stage: hr|manager|it
    python -m agents.smart_routing --self-check
"""
