...
(async () => {
  ...
  try {
    ...
    if (formVisible) {
      ...
      const submitted = await empPage.getByText(/submitted|thank/i).first().isVisible({ timeout: 5000 }).catch(() => false);
      if (submitted) {
    ...
    if (hasAsk) {
      ...
      const hasAnswer = await empPage.locator('.strip-body').isVisible({ timeout: 3000 }).catch(() => false);
      if (hasAnswer) {
    ...
    if (hasActions) {
      ...
      if (await rBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
        await rBtn.click();
        await mgrPage.waitForTimeout(1000);
        // Fill reason if a dialog/input appears
        const reasonInput = mgrPage.locator('input, textarea').filter({ hasText: '' }).last();
        const hasReason = await reasonInput.isVisible({ timeout: 2000 }).catch(() => false);
        if (hasReason) {
          await reasonInput.fill('Needs more detail.');
          const confirmBtn = mgrPage.getByRole('button', { name: /confirm|send|submit/i }).last();
      ...
      await mgrPage.goto(BASE + '/manager/kt-approvals', { waitUntil: 'networkidle', timeout: 20000 });
      await mgrPage.waitForTimeout(1000);
      const appBtns = mgrPage.getByRole('button', { name: /approve/i });
      ...
      // No KT items yet — agent service is down, tasks may not have been created
    ...
    if (hasClear) {
      ...
      pass('Step 7 - Manager clearance', 'Clearance approved');
    } else {
      fail('Step 7 - Manager clearance', 'No clearance button found');
    }
